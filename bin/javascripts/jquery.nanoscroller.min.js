...
(b.scrollHeight-b.clientHeight)*(a.paneH-a.sliderH)+"px"})},wheel:function(b){a.sliderY+=-b.wheelDeltaY||-b.delta;a.scroll();return!1}}};c.prototype.addEvents=function(){var a,b;a=this.events;b=this.pane;d(f).bind("resize",a.resize);this.slider.bind("mousedown",a.down);b.bind("mousedown",a.panedown);this.content.bind("scroll",a.scroll);f.addEventListener&&(b=b[0],b.addEventListener("mousewheel",a.wheel),b.addEventListener("DOMMouseScroll",a.wheel))};c.prototype.removeEvents=function(){var a,b;a=this.events;
b=this.pane;d(f).unbind("resize",a.resize);this.slider.unbind("mousedown",a.down);b.unbind("mousedown",a.panedown);this.content.unbind("scroll",a.scroll);f.addEventListener&&(b=b[0],b.removeEventListener("mousewheel",a.wheel),b.removeEventListener("DOMMouseScroll",a.wheel))};c.prototype.generate=function(){this.el.append('<div class="pane"><div class="slider"></div></div>');this.content=d(this.el.children()[0]);this.slider=this.el.find(".slider");this.pane=this.el.find(".pane");this.scrollW=h();if(0===
this.scrollbarWidth)this.scrollW=0;this.content.css({right:-this.scrollW+"px"});null!=d.browser.msie&&8>parseInt(d.browser.version,10)&&this.pane.hide()};c.prototype.reset=function(){if(!0===this.isDead)this.isDead=!1,this.pane.show(),this.addEvents();this.contentH=this.content[0].scrollHeight+this.scrollW;this.paneH=this.pane.outerHeight();this.sliderH=this.paneH/this.contentH*this.paneH;this.sliderH=Math.round(this.sliderH);this.scrollH=this.paneH-this.sliderH;this.slider.height(this.sliderH);this.paneH>=
this.content[0].scrollHeight?this.pane.hide():this.pane.show()};c.prototype.scroll=function(){var a;this.sliderY=Math.max(0,this.sliderY);this.sliderY=Math.min(this.scrollH,this.sliderY);a=this.paneH-this.contentH+this.scrollW;a=a*this.sliderY/this.scrollH;this.content.scrollTop(-a);return this.slider.css({top:this.sliderY})};c.prototype.scrollBottom=function(a){this.reset();this.content.scrollTop(this.contentH-this.content.height()-a)};c.prototype.scrollTop=function(a){this.reset();this.content.scrollTop(a+
0)};c.prototype.stop=function(){this.isDead=!0;this.removeEvents();this.pane.hide()};return c}();d.fn.nanoScroller=function(c){var a;c||(c={});a=this.data("scrollbar");if(void 0===a)a=new g(this),this.data({SCROLLBAR:a});else return c.scrollBottom?a.scrollBottom(c.scrollBottom):c.scrollTop?a.scrollTop(c.scrollTop):"bottom"===c.scroll?a.scrollBottom(0):"top"===c.scroll?a.scrollTop(0):!0===c.stop?a.stop():a.reset()}})(jQuery,window,document);